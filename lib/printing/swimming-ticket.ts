 ...
 *
 * Exports:
 *  - buildSwimmingTicketHtml(ticket)       → full HTML document for browser window.print()
 *  - buildSwimmingTicketTextLines(ticket)  → plain-text lines for ESC/POS thermal printer
 */

'use client'

export interface SwimmingTicketData {
  id: string
...
function ticketLabel(ticket: SwimmingTicketData): string {
...
export function buildSwimmingTicketTextLines(ticket: SwimmingTicketData): string[] {
  ...
  return [
    ...
    center('Enjoy your swim!'),
    '',
  ]
}

// ─── Full HTML document for browser window.print() ────────────────────────────
export function buildSwimmingTicketHtml(ticket: SwimmingTicketData): string {
  if (!ticket) return ''
...
<head>
  <meta charset="UTF-8" />
  <title>Swimming Ticket - ${ticketNo}</title>
  <style>
    @page {
      size: 80mm auto;
      margin: 4mm 4mm;
