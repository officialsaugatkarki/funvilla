...
import { Wifi, Wind, Fan } from 'lucide-react'
import { Canvas } from '@react-three/fiber'
import { OrbitControls, useGLTF, Environment } from '@react-three/drei'
import * as THREE from 'three'
import { RoomBookingForm } from './room-booking-form'

// Deep clone so two canvases never share geometry ArrayBuffers
...
function Model({ url }: { url: string }) {
  ...
  const clonedScene = useMemo(() => deepCloneScene(scene as unknown as THREE.Group), [scene])
...
const rooms = [
  ...
  },
  {
    ...
    photo: '/assets/room-standard.jpg',
    ...
    <section id="rooms" className="py-14">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        {rooms.map((room, index) => (
          ...
          >
            {/* Viewer: real photo on mobile, interactive 3D on desktop */}
            <div className="relative w-full sm:w-2/5 aspect-[4/3] sm:aspect-auto overflow-hidden bg-gray-100">
              {isMobile ? (
                // ── MOBILE: static photo, zero GPU cost, always correct ──
                <Image
                  src={room.photo}
                  alt={room.name}
                  fill
                ...
                // ── DESKTOP: interactive 3D model, unchanged ──
                <Canvas
